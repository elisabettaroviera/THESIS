...
statistical objective with six hard constraints that jointly enforce cardinality,
chromosomal balance, correlation independence, genomic-context coverage,
gene-level diversity, and methylation directionality.
...
CpG annotated to gene $g$ is selected, and are used both in the objective and
in constraint~\eqref{eq:c5}.
...
the absolute $\Delta M$ effect size, and the stability score from the feature
selection stage; the indicator $\mathbf{1}_{\{j \in \mathrm{CGC}\}}$ confers
an additional reward to loci annotated in the COSMIC Cancer Gene
Census~\cite{sondka2018cosmic}.
To promote genomic diversity beyond the statistical score, a diversification
...
\label{eq:phi}
  ...
  + \eta \sum_{g} y_g,
\end{equation}
which rewards the inclusion of promoter-proximal and CpG-island loci, and
penalises panels that concentrate their gene-level coverage in few loci
neighbourhoods.

\paragraph{Complete MILP formulation}

Combining the statistical objective~\eqref{eq:score} with the diversification
functional~\eqref{eq:phi}, the full optimisation problem reads:
\begin{align}
&\underset{x}{\text{max}} \quad
  \sum_{j \in \mathcal{J}} v_j\,x_j + \mu\,\hat{\Phi}(x,y) \notag \\[8pt]
&\text{s.t.} \notag \\[4pt]
&\quad \sum_{j \in \mathcal{J}} x_j = K
   \tag{C1}\label{eq:c1}\\[3pt]
&\quad x_i + x_j \le 1
   \qquad \forall\,(i,j):\,|r_{ij}| \ge 0.85
   \tag{C2}\label{eq:c2}\\[3pt]
&\quad \sum_{j \in \mathcal{J}_c} x_j \le \alpha_{\mathrm{chr}}\,K
   \qquad \forall\, c
   \tag{C3}\label{eq:c3}\\[3pt]
&\quad \sum_{j \in \mathcal{J}_\ell} x_j \le \alpha_\ell\,K
   \qquad \forall\,\ell \in \mathcal{L}
   \tag{C4}\label{eq:c4}\\[3pt]
&\quad \sum_{j:\,g(j)=g} x_j \le \delta_{\max}\,y_g
   \qquad \forall\, g
   \tag{C5}\label{eq:c5}\\[3pt]
&\quad \sum_{j:\,\Delta M_j > 0} x_j \ge h_{\min}
   \tag{C6}\label{eq:c6}\\[3pt]
&\quad x_j \in \{0,1\},\quad y_g \in \{0,1\}. \notag
\end{align}
Each constraint encodes a distinct biological or structural requirement.
Constraint~\eqref{eq:c1} fixes the panel cardinality exactly at $K = 50$.
Constraint~\eqref{eq:c2} prevents the simultaneous selection of locus pairs
whose Spearman correlation satisfies $|r_{ij}| \ge 0.85$, extending the
redundancy-pruning principle established in
Chapter~\ref{chap:feature_selection} to the compressed regime.
Constraint~\eqref{eq:c3} imposes a per-chromosome ceiling $\alpha_{\mathrm{chr}}$,
while constraint~\eqref{eq:c4} upper-bounds the proportion of selected loci
belonging to each genomic context $\ell \in \mathcal{L}$ (OpenSea, Island,
Shore, Shelf); together, these two constraints prevent the panel from becoming
genomically concentrated in a limited number of chromosomal or regulatory
regions. Constraint~\eqref{eq:c5} links the CpG selection variables to the
gene-level indicators through the parameter $\delta_{\max}$, which caps the
number of selected loci per gene and ensures that no single locus neighbourhood
dominates the panel. Finally, constraint~\eqref{eq:c6} enforces a lower bound
$h_{\min}$ on the count of hypermethylated loci, preventing polarity collapse
towards a purely hypomethylated solution.
...
a stable operating point, $\mu$ is swept over a discrete grid and the
resulting Pareto frontier between the two objectives is examined. The final
value is selected at the empirical knee of this curve, defined as the point
beyond which marginal gains in diversification no longer compensate for losses
in statistical score.
Given the moderate input dimensionality ($|\mathcal{J}| = 5{,}000$), the MILP
is solved to certified global optimality using the CBC solver via PuLP.
Solution stability across $\mu$ values is assessed by computing the Jaccard
similarity between each solution and the knee-point panel, confirming that
the selected subset is not sensitive to small perturbations of the
