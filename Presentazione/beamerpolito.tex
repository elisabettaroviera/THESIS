...
\begin{columns}[T]
  ...
  % COLONNA SINISTRA — PCA pre

  \begin{column}{0.46\textwidth}

    \centering

    \includegraphics[width=\textwidth]{images/plot_pca_precombat_by_dataset.pdf}

    {\scriptsize \textit{Pre-ComBat — colorato per dataset}}\\

    {\scriptsize Trustworthiness $(k=5)$ = \textbf{0.7308}}

  \end{column}

  % COLONNA DESTRA — PCA post

  \begin{column}{0.46\textwidth}

    \centering

    \includegraphics[width=\textwidth]{images/plot_pca_postcombat_by_dataset.pdf}

    {\scriptsize \textit{Post-ComBat — colorato per dataset}}\\

    {\scriptsize Trustworthiness $(k=5)$ = \textbf{0.7094}}
...
\begin{itemize}
  ...
  \item[\textbf{--}] \textbf{$\sim$174k CpG rimossi} (cross-reattivi, SNP-prossimali) $\rightarrow$ 485k CpG comuni
  \item[\textbf{--}] Trasformazione $|\beta - \mu_{\text{Normal}}|$: deviazione dal tessuto sano, coerente con la \textit{field cancerization}
  \item[\textbf{--}] \textbf{ComBat} per batch correction — stimato sul train, applicato al test: zero data leakage
...
\begin{tikzpicture}[
  ...
  box/.style={
    ...
    minimum width=7.5cm, minimum height=0.62cm,
    text centered, font=\small,
    draw=maincolor!60, fill=maincolor!8
  },
  highlight/.style={
    rectangle, rounded corners=3pt,
    minimum width=7.5cm, minimum height=0.62cm,
    text centered, font=\small,
    draw=maincolor, fill=maincolor!15, thick
  },
  final/.style={
    rectangle, rounded corners=3pt,
    minimum width=7.5cm, minimum height=0.62cm,
    text centered, font=\small,
    draw=maincolor, fill=maincolor!20, thick
  },
  start/.style={
    rectangle, rounded corners=3pt,
    minimum width=7.5cm, minimum height=0.62cm,
...
\node[start] (n0)
  {\textbf{485,453} CpG — post-preprocessing};

\node[box, below=of n0] (n1)
  {Filtro variabilità (Edgar $\beta$-filter) \quad $\rightarrow$ \textbf{284,310} CpG};

\node[box, below=of n1] (n2)
  {Filtro varianza M-space (data-driven) \quad $\rightarrow$ \textbf{270,094} CpG};

\node[highlight, below=of n2] (n3)
  {Stability ranking (RSKF) \quad $\rightarrow$ \textbf{15,659} CpG};

\node[highlight, below=of n3] (n4)
  {Correlation pruning (grafi) \quad $\rightarrow$ \textbf{15,523} CpG};

\node[final, below=of n4] (n5)
  {Diversificazione genomica \quad $\rightarrow$ \textbf{5,000} CpG};
...
\node[font=\scriptsize\itshape, text=maincolor, right=0.2cm of n3]
  {approfonditi};
\node[font=\scriptsize\itshape, text=maincolor, right=0.2cm of n4]
