...
The hyperparameters governing each stage of the pipeline 
— including variability thresholds, stability cutoffs, 
correlation bounds, and diversification budgets — were 
fixed based on domain knowledge and methodological 
precedent rather than systematic optimisation. 
A formal sensitivity analysis of these parameters 
lies outside the scope of the present work and 
is identified as a direction for future investigation 
in Chapter~\ref{chap:conclusion_future_work}.
...
$50{,}000$ CpGs ranked by 
$|\Delta M_{j}^{(s)}|$ within the training fold. 
CpGs outside this prefiltered subset are assigned 
$p_{j}^{(s)} = 1$, corresponding to the least favourable rank.
This two-stage procedure introduces a structural dependency 
between the prescreening criterion and the subsequent rank 
aggregation, since excluded CpGs receive a conservative rank 
by construction rather than by statistical evidence. 
The approximation is justified by the width of the prefilter: 
retaining the top $50{,}000$ candidates corresponds to 
approximately $6\%$ of the full feature space and 
substantially exceeds the final selection target of 
$5{,}000$ CpGs, thereby limiting the probability of 
excluding genuinely discriminative loci from downstream 
evaluation.
The Mann--Whitney test is adopted as a rank-based nonparametric procedure to avoid distributional assumptions and ensure robustness to residual deviations from normality in $M$-space~\cite{MannWhitney1947}.
...
the core statistical challenge posed by genome-wide DNA methylation
data in the HDLSS regime: identifying a compact, stable, and
biologically coherent CpG panel capable of characterising the
Normal--Adjacent epigenetic transition across independent cohorts.

The pipeline operates as a sequence of strictly train-only
dimensionality reduction stages, each targeting a distinct source
of noise or redundancy in the methylation feature space.
Table~\ref{tab:fs_reduction_steps_all} summarises the progressive
reduction from the post-preprocessing spaces — ranging from
$251{,}483$ to $530{,}683$ CpGs depending on platform — to the
final panels of $K = 5{,}000$ loci retained after variability
filtering, stability-based ranking, correlation pruning, and
constrained genomic diversification.
The resulting panels exhibit several desirable structural properties.
Directional stability scores are concentrated near unity across all
three cohorts, indicating that the selected CpGs express
reproducible Adjacent--Normal polarity across resampling splits.
Correlation-based redundancy pruning eliminates co-methylated
blocks, yielding non-redundant representatives of independent
epigenetic signals.
Genomic diversification enforces broad chromosomal and
regulatory-context coverage, reducing the risk that downstream
models capture locus-specific artefacts rather than system-level
drift patterns.


The inter-dataset evaluation of Section~\ref{sec:inter_dataset_fs}
reveals that cross-cohort replicability is partial and asymmetric.
GSE69914 and GSE225845 satisfy all criteria for the \emph{shared
drift structure} regime: their panels are mutually enriched beyond
random expectation, exhibit near-complete directional concordance
($c = 0.980$), and support bidirectional zero-shot transfer
(AUC $= 0.776$ and $0.794$).
...
This asymmetry has a direct methodological consequence.
Linear $\Delta\beta$ signatures, however stable within a cohort,
do not constitute a universally portable representation of the
Normal--Adjacent transition: their discriminative orientation
is modulated by platform architecture, sampling design, and
residual batch structure in ways that naive cross-cohort transfer
cannot accommodate.

The structural properties and cross-cohort limitations of the
final CpG panels jointly define the design space for the
downstream modelling strategy.
Within each cohort, the selected panels provide a low-dimensional,
stability-filtered, and biologically grounded input representation
for supervised classification of Normal versus Adjacent tissue.
Chapter~\ref{chap:model_training_evaluation} develops and evaluates a suite of
classifiers — ranging from regularised linear models to
ensemble methods — applied independently within each dataset,
assessing the degree to which the selected CpG signatures support
within-cohort discriminability.
The cross-cohort asymmetries documented here, however, motivate
an analysis that goes beyond standard intra-dataset evaluation.
Where drift orientation is preserved — as between GSE69914 and
GSE225845 — cross-dataset transfer of trained models is a
principled and tractable objective.
Where systematic inversion is present, as with GSE287331,
orientation-aware or cohort-adaptive strategies are required.
Chapter~\ref{chap:CpGPT} addresses this challenge through
CpGPT-based representation learning, investigating whether
higher-order methylation organisation, not captured by linear
$\Delta\beta$ projections, supports a more generalisable encoding
of the Normal--Adjacent epigenetic transition across cohorts with
