...
\begin{frame}{Il Tumore alla Mammella: una Sfida Globale}
...
\begin{columns}[T]
  ...
  \begin{column}{0.48\textwidth}
    ...
    {\scriptsize \textit{Tumore più comune per paese — GLOBOCAN 2022 \\ In rosa è riportato il tumore alla mammella}}
  ...
  \begin{column}{0.48\textwidth}
    ...
    {\scriptsize \textit{Immagini istologiche a confronto — Gadaleta et al., J Pathol, 2022}}\\
...
\node[hlstyle,     below=of n4] (n5) {Correlation pruning \hspace{0.8cm} $\rightarrow$ \hspace{0.4cm} \textbf{14,840} CpG};
...
\begin{columns}[T]
  ...
  \begin{column}{0.52\textwidth}
    ...
    \begin{tikzpicture}[
      ...
      \node[final] (fin) at (1.5,-3.2) {\textbf{14,840} CpG};
...
\section{Dal Problema di Ottimizzazione alla Soluzione}
...
& \sum_{j:\,g(j)=g} x_j \leq 2 \quad \forall\, g
  &\qquad& \textbf{(Bilanciamento genico)} \\[4pt]
...
\begin{itemize}
  ...
  \item \textbf{Ruolo:} mantenimento delle cellule staminali 
...
{\scriptsize BACC 0.91 \\ 13 geni COSMIC breast}
...
{\scriptsize
\textbf{Altri tumori}\\
Framework MILP applicato per esempio a colon e ovaio\\[0.6em]
...
{\scriptsize
\textit{Biologically-Constrained CpG Panel Selection via MILP for
Normal--Adjacent Breast Tissue Classification}\\[0.2em]
\textbf{IWBBIO 2026} — 13th International Work-Conference on
...
\begin{frame}{Appendice — Batch Effect}

\begin{figure}
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth, height=0.75\textheight, keepaspectratio]{images/GSE69914_GSE225845_GSE287331_07_tsne_by_dataset_NA.pdf}
        \caption{t-SNE coloured by dataset trustworthiness = 0.976}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth, height=0.75\textheight, keepaspectratio]{IMAGES/GSE69914_GSE225845_GSE287331_08_tsne_by_group_NA.pdf}
        \caption{t-SNE coloured by tissue state trustworthiness = 0.976}
    \end{subfigure}
\end{figure}
...
\begin{itemize}\setlength\itemsep{0.4em}
  ...
  \item $X\beta_j$ — effetto delle covariate biologiche protette
...
Modella il valore di \textbf{metilazione di ogni CpG} come somma di una media globale. Per stimare gli effetti di batch, usa una distribuzione a priori ricavata da tutti i CpG insieme. Una volta stimati, questi \textbf{effetti vengono rimossi dal segnale}.
}
...
% ---- RIGHT: immagine ----
\column{0.55\textwidth}
\includegraphics[width=\linewidth]{images/variance_filter_cutoff.pdf}

\end{columns}

\end{frame}

\begin{frame}{Appendice — Test di Mann-Whitney}

\textbf{Test non parametrico} per confrontare due gruppi indipendenti 
senza assumere normalità della distribuzione.

\textbf{Statistica test:}
\[
U_1 = \sum_{a \in X_{\text{adj}}} \sum_{b \in X_{\text{nor}}} 
\left( \mathbf{1}[a > b] + \tfrac{1}{2}\,\mathbf{1}[a = b] \right)
\]

\textbf{Approssimazione normale} (campioni grandi):
\[
z = \frac{U_1 - \mu_U}{\sigma_U}, \qquad
...
\textbf{Conclusione:} p-value basso $\Rightarrow$ la CpG è 
significativamente differenziata tra i due gruppi 
$\Rightarrow$ candidata rilevante per la feature selection.
